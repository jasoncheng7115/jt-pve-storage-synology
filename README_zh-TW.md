...
>
> 在那之後：**兩個節點**跑著不同的核心、掛著同一個 LUN，即時遷移的中斷時間約 3 毫秒，一台真的開機的 VM 持有該磁碟，以及**兩個 portal 的 multipath**——在負載中擋掉一條路徑，60 次讀取失敗 0 次，而且那條路徑自己恢復了。
>
> 它仍然是預發行版，誠實的理由是：**一個**機型、**一個** DSM 版本，而且從來沒有任何 HA 或雙控制器機箱接近過它。開始實際運行的第一個小時就找出七個「讀程式碼永遠讀不出來」的缺陷，之後一次稽核又找出四個——其中包括一組放錯檔案的憑證——所以請假設還有其他的。
>
