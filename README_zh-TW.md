...
每一列都在實機上、從網頁介面和指令列各跑過一次，虛擬機和容器都跑。不是「可以」的那幾列，都寫出原因。

| 操作 | 虛擬機 | 容器 |
|---|---|---|
| 建立、移除磁碟 | ✓ | ✓ |
| 精簡配置<br>每一顆磁碟都是 Btrfs thin LUN，所以它佔用的是已經寫入的量，不是建立時的大小。因為這樣可以超額配置 DSM 儲存空間，所以儲存空間的剩餘空間低於 `syno-min-free` 時會拒絕配置 | ✓ | ✓ |
| 擴充容量<br>容器的檔案系統也是線上長大 | ✓ | ✓ |
| 快照、倒回<br>在 block 層驗證過：寫入資料、拍快照、覆蓋成 0、倒回，原本的檢查碼回來了 | ✓ | ✓ |
| 含記憶體的快照<br>Proxmox VE 完全不會為容器拍記憶體快照 | ✓ | 不適用 |
| 完整複製<br>Proxmox VE 自己就拒絕對執行中的容器做完整複製 | ✓ | 需先停機 |
| 連結複製 | ✓ | ✓ |
| 轉換為範本 | ✓ | ✓ |
...
在請 NAS 建立任何東西之前，它會把 LUN 數量和 `max_iscsiluns` 比對，然後說出真正的原因才拒絕：這台 NAS 已經到了機型的 LUN 上限，只能先刪掉一些。剩下不到十六個時也會先警告一次，那時候還來得及規劃。

**這個計數包含不屬於這個 storage 的 LUN**：你自己建的、Virtual Machine Manager 用的，都吃同一個上限。也因為這樣，列出 LUN 時從不送 Synology 自己那份型態過濾條件，它會把這些物件藏起來。
...
Synology 有兩種都被叫做「HA」的架構，但它們是兩個不同的問題、有不同的答案。**兩種架構 plugin 都已有對應處理，但目前皆尚未完成實機驗證。**
...
plugin 把 DSM 儲存空間的 `size_total_byte` 與 `size_free_byte` **原封不動、以位元組**回報，兩個數字都沒有做任何運算。之後全都是顯示的事：Proxmox VE 每一級除以 1000（KB、MB、GB、TB），那是硬碟標容量的算法；DSM 每一級除以 1024，那是電腦內部計數的算法。連除四級之後差距大約 10%，而兩邊都把答案寫成 TB。
