...
| 離線遷移 | ✓ | ✓ |
| 備份、還原，`stop` 與 `suspend` 模式 | ✓ | ✓ |
| 備份、還原，`snapshot` 模式<br>容器不需要這個模式：`--mode suspend` 可以在容器執行中備份，只有短暫暫停。快照模式則要把快照掛起來讀檔案，而 Synology LUN 的快照沒有裝置可掛 | ✓ | 用 suspend |
| 把磁碟搬到其他 storage 型別<br>搬出去再搬回來，兩個方向都測 | ✓ | ✓ |
| 從**快照**做完整複製<br>完整複製是由 Proxmox VE 自己讀取來源，所以它需要在那個快照上有一個可以讀取的區塊裝置，而 Synology LUN 的快照給不了。請改用連結複製，或先把來源轉成範本 | 不支援 | 不支援 |

多重路徑撐過路徑故障，以及 CHAP，是這個 storage 的性質，不是 guest 的：節點上同一個 iSCSI 工作階段同時服務兩種 guest。兩者都驗證過。
