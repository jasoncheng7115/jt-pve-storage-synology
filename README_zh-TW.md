...
>
> **第二個機型與第二個 DSM 版本已經回答了**（唯讀）：一台 **DS925+，DSM 7.3.2-86009 Update 4**。這個 plugin 用到的每一個 API 都存在，版本與 CGI 路徑相同，各機型上限讀到的數字一致，而把關 `pvesm add` 的 Btrfs 與支援度檢查都通過。**資料路徑沒有在那台上跑過**——它拒絕 3260 埠——所以這是答案的 API 那一半，不是全部。
>
> 仍然該誠實說的是：**資料路徑**只在**一個**機型上跑過，從來沒有任何 Synology HA 或雙控制器機箱接近過它，而那個 DSM 帳號需要管理員權限——因為 DSM 7.1.1 沒有更窄的選擇，非管理員連登入都過不了。從實際運行中找出大約三十個「讀程式碼永遠讀不出來」的缺陷，所以請假設還有更多。
>
> `1.0.0` 在等的是：**資料路徑**的第二個機型，以及確定最小的 DSM 權限。[docs/TESTING_zh-TW.md](docs/TESTING_zh-TW.md) 是那份「哪些驗證過、哪些沒有」的登記簿，在信任這個東西之前值得讀一遍。
...
**實際運行找出五個缺陷，五個都修好了**——在 0.6.6 到 0.6.13 之間，每一個都附帶一個「拿掉修正就會失敗」的測試。它們的形狀完全一樣：從 shell 驅動時可以動，從操作者真正使用的介面驅動時就壞掉。哪些驗證過、哪些沒有，登記簿在 [docs/TESTING_zh-TW.md](docs/TESTING_zh-TW.md)。
