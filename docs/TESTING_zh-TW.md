...
| R-27 | **未解決，2026-08-07 對 0.6.24 重跑發版清單時找到的真缺陷**。一顆 LUN 被刪除、NAS 重用它的 mapping index 之後，節點也重用了同一個 sd 節點：核心在 rescan 時重讀 VPD，把 `/sys/block/<sd>/device/wwid` 更新成**新的** LUN，而 **multipathd 從來沒有重讀那條路徑**，還為那顆已刪除的 LUN 守著一個 map。兩邊並排量過——核心是 `naa.60014052e46494ed5667d4a29dbe0dd9`，multipathd 是 `36001405bbc484c9dc23cd4accd8f7fd1`，同一個 `sde`。`ensure_map` 於是在等一個永遠建不起來的 map | **`qm move_disk` 搬回這個 storage、以及 `qmrestore`，在任何曾經在該處刪過 LUN 的節點上都會重現失敗**——連續四次，每次都是全新的 WWID。這兩項都在發版清單上（B3、E4）。**它不是資料風險**：plugin 是拒絕而不是拿那個裝置去用，那正是規則 48 在發揮作用。已驗證的繞法：刪掉那個 sd 節點（`echo 1 > /sys/block/<sd>/device/delete`）再 `iscsiadm -m session --rescan`。**沒有**用的做法（試過並已回復）：把路徑從 multipathd 拿掉、用名稱 flush 那個屍體 map、再把路徑加回去——屍體還在。重現環境：pc-pve1、PVE 9.2.5、`find_multipaths no`、DS918+／DSM 7.4.1 |
