...
| R-27 | **OPEN, and a real defect found on 2026-08-07 by re-running the release checklist against 0.6.24.** After a LUN is deleted and the NAS reuses its mapping index, the node reuses the sd node too: the kernel re-reads the VPD on rescan and updates `/sys/block/<sd>/device/wwid` to the **new** LUN, while **multipathd never re-reads the path** and goes on holding a map for the deleted one. Measured side by side — kernel `naa.60014052e46494ed5667d4a29dbe0dd9`, multipathd `36001405bbc484c9dc23cd4accd8f7fd1`, same `sde`. `ensure_map` then waits for a map that can never be built | **`qm move_disk` onto this storage and `qmrestore` fail reproducibly** on any node that has previously deleted a LUN there — four times in a row, each with a fresh WWID. Both are release-checklist rows (B3, E4). **It is not a data risk**: the plugin refuses rather than using the device, which is rule 48 doing its job. Verified workaround: delete the sd node (`echo 1 > /sys/block/<sd>/device/delete`) then `iscsiadm -m session --rescan`. What did **not** work, tried and reverted: dropping the path from multipathd, flushing the corpse map by name, re-adding the path — the corpse survives. Reproduced on pc-pve1, PVE 9.2.5, `find_multipaths no`, DS918+/DSM 7.4.1 |
